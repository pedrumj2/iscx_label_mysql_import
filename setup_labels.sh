...
#  
#  
#eval "mysql -u root -p'$1' $i -e \"insert into D11.ipaddr(ipValue)
#        select ipValue from $i.ipaddr
#        where ipValue not in 
#        (
#      	  select ipValue from  $i.ipaddr
#        );\""
  ...
  eval "mysql -u root -p'$1' $i -e \"insert into labels(source, dest, startTime, endTime, tag)
                                      ...
                                      inner join D11.ipaddr as S
                                      on ${i}CSV.source = S.ipValue
                                      inner join D11.ipaddr as D
